...
const Dynamo = {
  ...
  async addPlayerPoint(connectionID, room, username, next) {
    console.log(`ADD PLAYER POINT room:${room} username:${username}`);
    return await client
      .query({
        TableName: playerTable,
      ...
      .then((result) => {
        result.Items.forEach((item) => {
          console.log(`FOUND PLAYER POINT :${item.data}`);
          if (item.data.points) item.data.points = item.data.points + 1;
          else item.data.points = 1;
          client
            .put({
              TableName: playerTable,
              Item: {
